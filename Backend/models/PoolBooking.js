...
poolBookingSchema.pre('save', function(next) {
  if (!this.totalAmount || this.totalAmount <= 0) {
    ...
    if (this.checkIn && this.checkOut) {
      const durationHours = (new Date(this.checkOut) - new Date(this.checkIn)) / (1000 * 60 * 60);
    ...
    console.log(`Auto-calculated totalAmount: ${peopleCount} people × Rs.${totalAmount/peopleCount} = Rs.${this.totalAmount}`);
  }
