...
async function updateExistingEvents() {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/hotel-system');
    ...
    console.log(`Found ${eventsWithoutStatus.length} events without status field`);
    ...
    if (eventsWithoutStatus.length > 0) {
      ...
      console.log(`Updated ${result.modifiedCount} events to have 'pending' status`);
    }
    ...
    pendingEvents.forEach(event => {
      console.log(`- ${event.eventId || event._id}: ${event.name || 'No name'} (${event.eventType || 'No type'})`);
    });
