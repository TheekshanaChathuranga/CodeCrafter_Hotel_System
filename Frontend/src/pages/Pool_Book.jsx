...
const Pool_Book = () => {
  ...
  useEffect(() => {
    const fetchPools = async () => {
      try {
        ...
        const availablePools = res.data.filter(pool => pool.poolStatus === "Available");
  ...
  const handleDateChange = async (e, poolId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/api/poolBookings/${poolId}/${selectedDate}`);
  ...
  const handleConfirmYes = async () => {
    ...
    if (
      !data?.fullName || !data?.date || !data?.guestCount ||
      !data?.proof || !data?.checkInTime || !data?.checkOutTime ||
      !data?.phoneNumber || isNaN(guestCount) || guestCount < 1
    ) {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/api/pool-bookings/${pool._id}/${data.date}`);
      const existingCount = res.data.totalGuests || 0;

      if (existingCount + guestCount > pool.capacity) {
        setBookingStatus(prev => ({
          ...prev,
          [pool._id]: { message: "Pool capacity exceeded for this date", error: true },
        }));
        return;
      }
  ...
  return (
    ...
    <div>
      ...
      <div className="bg-gray-100 min-h-screen px-4 md:px-10 py-6">
        ...
        {pools.map(pool => (
          <div key={pool._id} className="bg-white shadow-lg rounded-xl mb-10 overflow-hidden">
            <div className="md:flex">
              ...
              <div className="p-6 flex-1">
                ...
                <form onSubmit={(e) => handleBookingRequest(e, pool)} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      />
                      {availability === "full" && (
                        <p className="text-red-600 font-medium mt-1">❌ Fully Booked for this date</p>
                      )}
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2">
                      Upload Proof of Payment (Image/PDF) <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="file"
                      name="proof"
                      ...
                      required
                    />
                    <p className="text-sm text-gray-500 mt-1">Max 5MB. Formats: JPG, PNG, PDF</p>
                  </div>
                    
                  </div>
                  {/* Request */}
                  <div>
                    <label className="block text-gray-700 mb-2">Specific Request</label>
                    <textarea name="request" rows="3" value={formData[pool._id]?.request || ""} onChange={(e) => handleChange(e, pool._id)} className="w-full px-4 py-2 border rounded-lg" placeholder="Optional request..."></textarea>
                  </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full text-center">
        ...
        <p className="text-gray-700 mb-2">
          Date: <span className="font-medium text-blue-600">
          {formData[confirmData?.pool?._id]?.date || "N/A"}
        </span>
      </p>
      <p className="text-gray-700 mb-6">Do you want to confirm this booking?</p>
