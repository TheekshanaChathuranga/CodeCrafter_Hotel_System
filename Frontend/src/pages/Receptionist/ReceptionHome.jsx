...
const ReceptionHome = () => {
  ...
  const getBookingData = (booking) => {
    return {
      guestName:
        booking?.guestDetails?.name || booking?.fullName || "Unknown Guest",
      roomNumber:
        booking?.bookingDetails?.roomNumber || booking?.roomNumber || "N/A",
      roomType:
        booking?.bookingDetails?.roomType || booking?.roomType || "Unknown",
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (!connectionTest.success) {
        setError(
          `Backend connection failed: ${connectionTest.error}. Please ensure the backend server is running on port 5000.`
        );
        setLoading(false);
        return;
      }
      ...
      if (
        ...
        setError(
          "API endpoints not found. Please check if the backend routes are properly configured."
        );
  ...
  const loadSelectedDateBookings = async () => {
    ...
    try {
      const bookingsForDate = await receptionAPI.getBookingsForDate(
        selectedDate
      );
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold text-gray-900">
            {user?.role === 'admin' ? 'Dashboard' : 'Reception Dashboard'}
          </h1>
            ...
            <div className="bg-white overflow-x-auto">
              {todaysBookings.length > 0 ? (
                ...
                <div className="px-6 py-8 text-center">
                  ...
                  <button
                    onClick={() =>
                      (window.location.href = "/receptionist/roomBooking")
                    }
                ...
                {dashboardStats?.availableRooms?.map((room, index) => (
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    <div className="mt-2 text-xs text-gray-500 text-center">
                      {Math.round((room.available / room.total) * 100)}%
                      available
                    </div>
                  </div>
                )) || (
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div className="px-4 py-5 sm:p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <button
                  onClick={() => {
                    ...
                    if (confirmedBookings.length > 0) {
                      handleBookingStatusUpdate(
                        confirmedBookings[0]._id,
                        "checked-in"
                      );
                    }
                  }}
                  disabled={
                    !todaysBookings.some((b) => b.status === "confirmed") ||
                    refreshing
                  }
                ...
                <button
                  onClick={() => {
                    const checkedInBookings = todaysBookings.filter(
                      (b) => b.status === "checked-in"
                    );
                    if (checkedInBookings.length > 0) {
                      handleBookingStatusUpdate(
                        checkedInBookings[0]._id,
                        "checked-out"
                      );
                    }
                  }}
                  disabled={
                    !todaysBookings.some((b) => b.status === "checked-in") ||
                    refreshing
                  }
