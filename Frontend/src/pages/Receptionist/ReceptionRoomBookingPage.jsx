...
const ReceptionRoomBookingPage = () => {
  ...
  const fetchAvailableRooms = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/receptionRooms/available', {
        params: {
  ...
  const confirmBooking = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/roomBookings', bookingData);
