                ...
                <div className="space-y-1 max-h-16 overflow-y-auto">
                  {dayBookings.slice(0, 2).map(booking => {
                    ...
                    return (
                      <div
                        key={booking._id}
                        className={`text-xs p-1 rounded border-l-4 truncate ${getBookingColor(bookingData.roomType)}`}
            ...
            {selectedBookings.length === 0 ? (
              ...
              <div className="space-y-3">
                {selectedBookings.map(booking => {
                  const bookingData = getBookingData(booking);
                  return (
                    <div key={booking._id} className={`p-3 rounded-lg border-l-4 ${getBookingColor(bookingData.roomType)}`}>
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-medium">{bookingData.guestName}</h4>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                        <div>
                          <span className="text-gray-500">Check-in:</span> {bookingData.checkIn ? format(parseISO(bookingData.checkIn), 'MMM d') : 'N/A'}
                        </div>
                        <div>
                          <span className="text-gray-500">Check-out:</span> {bookingData.checkOut ? format(parseISO(bookingData.checkOut), 'MMM d') : 'N/A'}
                        </div>
                        <div>
                          <span className="text-gray-500">Contact:</span> {bookingData.mobile}
                        </div>
                        <div>
                          <span className="text-gray-500">Amount:</span> Rs. {bookingData.totalAmount}
                        </div>
                      </div>
                    </div>
                  );
