...
export const receptionAPI = {
  ...
  getAllBookings: async () => {
    try {
      ...
      try {
        ...
        console.error('Error fetching bookings from /receptionBookings:', fallbackError);
  ...
  updateBookingStatus: async (bookingId, status) => {
    try {
      ...
      // Fallback to try receptionBookings
      try {
        const response = await api.patch(`/receptionBookings/${bookingId}`, { status });
        return response.data;
      } catch (fallbackError) {
        console.error('Error updating booking status via /receptionBookings:', fallbackError);
  ...
  getDashboardStats: async () => {
    try {
      ...
      console.error('Dashboard stats endpoint not available, calculating locally:', error);
      ...
      try {
        ...
        console.error('Unable to fetch bookings for local calculation:', bookingsError);
  ...
  calculateStatsLocally: (allBookings) => {
    ...
    const hasActivityToday = todaysCheckIns > 0 || todaysCheckOuts > 0 || currentlyOccupied > 0;
    ...
    if (!hasActivityToday) {
      ...
      displayStats = {
        ...
        currentlyOccupied: allBookings.filter(booking => 
  ...
  getBookingsForDate: async (date) => {
    try {
      ...
      console.error('Dashboard bookings endpoint not available, filtering locally:', error);
